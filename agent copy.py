...
def get_endpoint_and_payload(action, context):
    ...
    if action_type == "vector_search":
        return VECTOR_SEARCH_URL, {"query": query, "k": 128, "bge_threshold": 0.2, "semantic_threshold": 0.5 }
    elif action_type == "entity_hybrid":
        return HYBRID_SEARCH_URL, {"query": query, "k": 128, "bge_threshold": 0.2, "semantic_threshold": 0.5 }
...
async def agent_loop(user_query):
    ...
    reasoning_log = []
    previous_hypotheses = []
    accumulated_supporting = [] 
    agent_thoughts: str = ""         # <-- новый список
